    ...
    $.fn.imageUploader = function (options) {
        ...
        let validateExtension = function (file) {
            if (plugin.settings.extensions.indexOf(file.name.toLowerCase().replace(new RegExp('^.*\\.'), '.')) < 0) {
                ...
                alert(`Du lieu hinh anh "${file.name}" khong khop voi cac phan mo rong tep duoc chap nhan: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMIME = function (file) {
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                ...
                alert(`Du lieu hinh anh "${file.name}" khong khop voi cac loai hinh anh duoc chap nhan: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            if (file.size > plugin.settings.maxSize) {
                ...
                alert(`Du lieu hinh anh "${file.name}" vuot qua kich thuoc toi da ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                ...
                alert(`Du lieu hinh anh "${file.name}" khong the duoc them boi vi so luong toi duoc chap nhan da la ${plugin.settings.maxFiles} hinh anh`); // Update
        ...
        let setPreview = function ($container, files) {
            ...
            $(files).each(function (i, file) {
                ...
                reader.onload = function (e) {
                    ...
                    // Set preview
                    $uploadedContainer.append(createImg(/*URL.createObjectURL(file)*/e.target.result, dataTransfer.items.length - 1), false);
                }
